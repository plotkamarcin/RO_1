...
public class HarrisEdgeExtractor {
	...
	public int[] process() {
		...
		for (int x = 0; x < width; x++) {
			...
			for (int y = 0; y < height; y++) {
				...
				if (val > threshold)
					...
					hcr[y * width + x] = 0;
				// System.out.println("diffx:"+ diffx[y * width + x] + "
				// diffy:"+ diffy[y * width + x] + " diffxy:"+ diffxy[y * width
				// + x] );
		...
		for (int x = 0; x < width; x++) {
			...
			for (int y = 0; y < height; y++) {
				if ((nm[y * width + x]) == 0)
					val = ((input[y * width + x] & 0xff) + 255) / 2;// hcr[y *
																	// width +
																	// x];//&0xff;
				else {
					// System.out.println("Got value " + (nm[y * width +
					// x]&0xff));
					val = 0;
